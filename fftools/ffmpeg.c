...
static int read_image_new_b(uint8_t *data, float *buf, float off, int width, int height, int stride, int ref_flag)
{
	...
	if (first_flag) {
		first_flag = 0;
		reserved_ref = width * height * DECODE_FRAME_NUM_PER_GOP * 3 / 2;
		reserved_dis = width * height * DECODE_FRAME_NUM_PER_GOP * 3 / 2;
	}
	...
	for (i = 0; i < height; i++) {
		...
		if (ref_flag)
			memcpy(tmp_buf, data + DECODE_FRAME_NUM_PER_GOP*width*height*3/2 - reserved_ref + i * width, width);
		else
			memcpy(tmp_buf, data + DECODE_FRAME_NUM_PER_GOP*width*height*3/2 - reserved_dis + i * width, width);
...
static int encode_frame(InputStreamInfo *p_input_stream_info, EncodeInfo *p_enc_info, float crf_val, int filtered_flag, MemInfo *pmeminfo)
{
	...
	int encode_total_num = filtered_flag ? 10 : DECODE_FRAME_NUM_PER_GOP;
...
static int decode_write_frame(FILE *pOutput_File, AVCodecContext *avctx,
                              AVFrame *frame, int *frame_count, AVPacket *pkt, int last, MemInfo *pmeminfo, int filtered_flag)
{
    int i;
    int idx;
    int color_idx;
...
static int decode_encoded_h264_rawdata(DecEncH264FmtInfo *pinfo, MemInfo *pmeminfo)
{
	...
	while (pinfo->uDataSize > 0) {
		...
		if(decode_write_frame(pinfo->outputfp, pinfo->codecCtx, pinfo->frame, &frame_count, &(pinfo->pkt), 0, pmeminfo, 0) < 0){
			exit(1);
		}
	} 
	...
	pinfo->pkt.size = 0;
	pinfo->pkt.data = NULL;
	decode_write_frame(pinfo->outputfp, pinfo->codecCtx, pinfo->frame, &frame_count, &pinfo->pkt, 0, pmeminfo, 0);
...
static int unsharp_decoded_yuv(UnsharpFilterInfo *pfilterinfo, const MemInfo *pmeminfo, const InputStreamInfo *p_input_stream_info, FILE *fp_filter)
{
	int ret = 0;
	...
	while (read_yuv_data_to_buf(pfilterinfo->frame_buffer_in, pmeminfo->pVideoBuffer, &(pfilterinfo->frame_in), frameWidth, frameHeight)) {
...
static int enc_filtered_yuv_to_264(MemInfo *pmeminfo, float crf_val, const InputStreamInfo *p_input_stream_info)
{
	...
	printf("linesize %d %d %d width %d height %d\n", 
	...
	for (int i = 0; i < 10; i++) {
		...
		x4->params.rc.f_rf_constant = x4->crf = (float)crf_val;
		x264_encoder_reconfig(x4->enc, &(x4->params));
...
static int decode_filtered_encoded_h264_rawdata(MemInfo *pmeminfo)
{
	...
	while (uDataSize > 0) {
		...
		if (decode_write_frame(fp, pcodecctx, pframe, &frame_count, &pkt, 0, pmeminfo, 1) < 0) {
			exit(1);
		}
	}
	...
	pkt.size = 0;
	pkt.data = NULL;
	decode_write_frame(fp, pcodecctx, pframe, &frame_count, &pkt, 0, pmeminfo, 1);
...
static int FristPartofPreProcess(char *filename)
{
	...
	UnsharpFilterInfo filterinfo = {0};
	...
	filterinfo.filter_descr = "unsharp=luma_msize_x=7:luma_msize_y=7:luma_amount=-1.0";

	//allocate meminfo
	meminfo.pVideoBuffer 		= (uint8_t *)malloc(FHD_BUFFER_SIZE);
	meminfo.pEncodeVideoBuffer 	= (uint8_t *)malloc(1024*1024*10);
	meminfo.pDecodeVideoBuffer 	= (uint8_t *)malloc(FHD_BUFFER_SIZE);
	meminfo.pVideoBuffer2		= (uint8_t *)malloc(FHD_BUFFER_SIZE / 5);
	meminfo.pEncodeVideoBuffer2 = (uint8_t *)malloc(1024*1024*10);
	meminfo.pDecodeVideoBuffer2 = (uint8_t *)malloc(FHD_BUFFER_SIZE / 5);
		...
		do {
			if (p_input_stream_info->p_pkt->stream_index == p_input_stream_info->video_stream_idx) {
				...
				while (ret >= 0) {
					...
					av_image_copy(decinfo.video_dst_data, decinfo.video_dst_linesize,
								  (const uint8_t *)(p_input_stream_info->p_frame->data),
								  p_input_stream_info->p_frame->linesize,
								  decinfo.pix_fmt, decinfo.width, decinfo.height);
					sharpness = get_unsharp_val(decinfo.video_dst_data[0],
									decinfo.width, decinfo.height, 1.0, 5, 5);
					fwrite(decinfo.video_dst_data[0], 1, decinfo.video_dst_bufsize, inputpar.video_dst_file);
					if (framenum < DECODE_FRAME_NUM_PER_GOP) {
						memcpy(meminfo.pVideoBuffer + framenum * decinfo.width * decinfo.height * 3 / 2,
								decinfo.video_dst_data[0], decinfo.width * decinfo.height * 3 / 2);
					} else if (framenum >= MIN_NUM_OF_PER_GOP) {
...
NEXT:
	for (int crf = 18; crf <= 19; crf++) {
    	...
    	compute_vmaf_prepare(&s, &vmaf_width, &vmaf_height, 
    						Dec264FmtInfo.frame->width, Dec264FmtInfo.frame->height, 
    						meminfo.pVideoBuffer, meminfo.pDecodeVideoBuffer);
		...
		printf("compute_vmaf done vmaf_score %f saved_data_size %d\n", vmaf_score, saved_data_size);
		saved_data_size = 0;
	}
	...
	for (int crf = 18; crf < 19; crf++) {
    	...
    	ret = enc_filtered_yuv_to_264(&meminfo, (float)(crf), p_input_stream_info);
    	...
    	compute_vmaf_prepare(&s, &vmaf_width, &vmaf_height, 
    						...
    						meminfo.pVideoBuffer2, meminfo.pDecodeVideoBuffer2);
		compute_vmaf(&vmaf_score, fmt, vmaf_width, vmaf_height, read_frame_new, s, model_path, log_file, NULL,
        				1/*disable_clip*/, 1/*disable_avx*/, 0/*enable_transform*/, 0/*phone_model*/, 0/*do_psnr*/, 0/*do_ssim*/, 
        				0/*do_ms_ssim*/, pool_method, 0/*n_thread*/, 5/*n_subsample*/, 0/*enable_conf_interval*/);
...
int main(int argc, char **argv)
{
	...
	printf("start the fristPartofPreProcess, argv[1] %s argv[2] %s\n", argv[1], argv[2]);
